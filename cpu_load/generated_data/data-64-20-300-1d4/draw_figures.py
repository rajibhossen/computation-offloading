...
df = pd.read_csv('data-64-20-300-1d4.csv')
df['arrival_time'] = pd.to_datetime(df['arrival_time'], unit='s')
print(df.head())
line = df.drop(['client_id', 'job_id', 'end_time', 'queue_time', 'total_time'], axis=1).plot(x='arrival_time', y=['job_time'])
fig = line.get_figure()
fig.savefig('job_times.pdf')
